  ...
  */
  ...
  BLE_item_data = BLE_data.substring(bleindex+2,bleindex+2 + (currItemlen*2));
      ...
      if(BLE_item_type=="09")
	  {
	      ...
	      if(name.substring(1,9)=="3EButton"){
		  
          console.log('parsing message',  messageTime,pktmessage.length);
	  ...
	  else if(BLE_item_type == "FF")
	  {
		  ...
		  cell0.innerHTML = " <h3off font-size = 500%> Off "+ msgQueue.length+"</h3> " ;
		  }
		  else
		  {
		  cell0.style.fontSize="20pt";
		  ...
		  cell0.innerHTML = "<h3on font-size = 500%> On" + msgQueue.length+"</h3> " ;
		  }
		  }
	  }
	   /*
...
function onMessageArrived(message) {
  ...
  len=header[0].length;
  pktmessage = header[0].substring(len-4,len)+pktmessage[1]; 
  console.log('receice message', messageTime,pktmessage.length);
  parsingPacket(pktmessage,messageTime);
