...
function parsingPacket(pktmessage,messageTime)
{
  ...
  if(pktcmd=="00"){
	   ...
	   var a=0;
	   console.log('rowlen = ', rowlen, a, a<rowlen);
	   for(a=0;a<rowlen;a++)
	   {
		    row = table.rows[a];
			var cell = table.rows[a].cells[1];
			cell.style.fontSize="20pt";
		    cell.style.height="30pt";
		    cell.style.bgColor="blue";
		    cell.innerHTML = " <h3off font-size = 500%> Off </h3> " ;
	   }
	   
  }
  else
  {
  console.log('parsing phase(1) cmd = 1');	  
  pktData = pktmessage.substring(pktindex+14,pktindex+(pktlen*2));
  ...
  console.log('parsing phase(2) itemlen = ', currItemlen);	  
  if(currItemlen>0)
  {
  ...
  console.log('parsing phase(3) type = ', BLE_item_type);	 
      if(BLE_item_type=="09")
	  {
		  console.log('parsing message(09)');
	      var name = ascii2str(BLE_item_data);
	      if(name.substring(1,3)=="3E"){
			  //(1,3)= '3E'
		  console.log('3EButton matched ');
		  hash_code = hashCode(BLE_address);
          if (hashtable[hash_code]==null)
          {
		  console.log('null');
		  usertable[hash_code] = user_count;

		  currentUser = user_count;
		  console.log('(4) set user = ', currentUser);
		  hashtable[hash_code] = 1;
		  ...
		  {
		  currentUser = usertable[hash_code];
		  console.log('(4) set user = ', currentUser);
		  }
		  
		  console.log('(5) check current user = ', currentUser);
	      var row = table.rows[currentUser];
		  var cell0 = table.rows[currentUser].cells[0];
		  if(cell0.innerHTML=="")
		  {
		  cell0.style.height="30pt";
		  cell0.style.fontSize="20pt";
		  cell0.innerHTML = " <h3name font-size = 200%>" +ascii2str(BLE_item_data)+"</h3off>";
		  }
		  }
		  else
		  currentUser=-1;
	  ...
	  {
		  console.log('parsing message(FF) user = ',currentUser);
	      if(currentUser!=10){
	      var row = table.rows[currentUser];
		  var cell0 = table.rows[currentUser].cells[1];
		  if(BLE_item_data.substring(currItemlen*2-1,currItemlen*2)=="0")
		  {
		  console.log('parsing message(4-off)');
		  cell0.style.fontSize="20pt";
		  cell0.style.height="30pt";
		  cell0.style.bgColor="blue";
		  cell0.innerHTML = " <h3off font-size = 500%> Off </h3> " ;
		  }
		  else
		  {
		  console.log('parsing message(4-on)');
		  cell0.style.fontSize="20pt";
		  cell0.style.height="30pt";
		  cell0.style.bgColor="red";
		  cell0.innerHTML = "<h3on font-size = 500%> On </h3> " ;
		  }
		  }
	  }
	   /*
	  else
	  {
          ...
          cell3.innerHTML = table.rows.length
...
function showData()
{
  ...
  {
  console.log('poping Message from queue : remain ', msgQueue.length);
